...
from agents.clients import AnalyticsInternalClient, JarvisInternalClient
from .marketing_channels.buffer_client import BufferClient, get_buffer_client
from .marketing_channels.instagram_client import InstagramClient, get_instagram_client
from .marketing_channels.whatsapp_client import WhatsAppClient, get_whatsapp_client
...
async def generate_campaign_draft(
    ...
    *,
    ...
    analytics_client = AnalyticsInternalClient()
    c_res = await analytics_client.create_campaign(
        title=title,
        channel=channel,
        status="pending_approval",
        content=content,
    ...
    queue_item_id = q_res.get("item", {}).get("id")
...
async def execute_approved_campaign(
    ...
    *,
    ...
    analytics_client = AnalyticsInternalClient()
    c_res = await analytics_client.get_campaign(campaign_id)
    campaign = c_res.get("campaign")
    ...
    # Step 1: If promo code discount attached, create promotion via internal analytics API
    ...
    if discount_config:
        ...
        try:
            await analytics_client.create_promotion(
                code=code,
    ...
    try:
        ...
        await analytics_client.patch_campaign(
            campaign_id,
            status="published",
