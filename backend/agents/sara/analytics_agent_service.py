...
from agents.clients import JarvisInternalClient, AnalyticsInternalClient
...
def convert_to_usd(amount: Optional[float], currency: Optional[str]) -> float:
    """Converts a local currency transaction amount to USD equivalent using deterministic FX table."""
    if amount is None:
        return 0.0
...
async def _enqueue_anomaly_dedup(
    ...
    *,
    ...
    """Helper ensuring we enqueue anomaly work items via internal JARVIS client."""
...
async def evaluate_system_anomalies(db=None) -> List[AnomalyEvaluationResult]:
    ...
    promos_data = await analytics_client.get_promotions()
    for promo in promos_data.get("promotions", []):
