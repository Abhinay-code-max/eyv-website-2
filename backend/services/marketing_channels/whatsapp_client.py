"""
WhatsApp Business Cloud API Integration - Task A.4.3 of EYV Marketing Agent (Bob).

Direct client for Meta WhatsApp Cloud API (https://graph.facebook.com/v19.0).
Implements template message dispatch, text messaging, and webhook payload parsing.
Reuses webhook-only architecture and reply correlation via message context.
"""
import logging
import os
...
class WhatsAppClient:
    ...
    async def send_text_message(
        ...
        """Sends a text message to a user phone number (E.164 format without +)."""
    ...
    async def send_template_message(
        ...
        """Dispatches an approved WhatsApp Business template message."""
        clean_phone = to_phone.replace("+", "").replace("-", "").replace(" ", "").strip()
        ...
        if response.status_code != 200:
            logger.error(f"WhatsApp API error: {response.status_code} - {response.text}")
            raise WhatsAppClientError(f"WhatsApp template error HTTP {response.status_code}: {response.text[:200]}")
